...
public class CharacterMotorC : MonoBehaviour {
	...
	public static bool playerSprint;
	...
	void FixedUpdate () {
		...
		if(!playerHit)
		{
			...
			if(playerSprint == true && Input.GetAxis ("Vertical") >= 0.8f)
			{
				movement.maxForwardSpeed = sprintSpeed;
				cameraMotionBlur.enabled = true;
			}
			else
			{
				movement.maxForwardSpeed = walkSpeed;
				playerSprint = false;
		...
		{
			movement.maxForwardSpeed = slowSpeed;
			movement.maxSidewaysSpeed= slowSpeed;
			movement.maxBackwardsSpeed= slowSpeed;
			playerSprint = false;
			...
			if(slowTimer <= 0f)
			{
				movement.maxSidewaysSpeed= walkSpeed;
				movement.maxBackwardsSpeed= walkSpeed;
				slowTimer = slowMaxTimer;
				playerHit = false; 
			}
		}
